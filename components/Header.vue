      ...
      <div class="flex flex-row justify-end">
        ...
        <ClientOnly>
          <UButton :icon="isDark ? 'i-heroicons-moon-20-solid' : 'i-heroicons-sun-20-solid'" color="gray"
            variant="ghost" aria-label="Theme" class="px-2.5" @click="isDark = !isDark" />
          <template #fallback>
            <div class="w-5 h-5" />
          </template>
        </ClientOnly>
        ...
        <template v-if="authenticated" class="float-right">
          <UDropdown :items="finalDropItems" :popper="{ placement: 'bottom-start' }">
            <template v-if="avatar">
              <UAvatar :src="'data:image/png;base64,' + avatar" alt="Avatar" icon="i-material-symbols-account-circle"
                size="sm" color="gray" variant="ghost" :label="fullName" aria-label="Theme" />
            </template>
            <template v-else>
              <UButton :icon="'i-material-symbols-account-circle'" color="gray" variant="ghost" size="md"
                aria-label="Theme" />
            </template>
            ...
            <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
              ...
              <template #footer>
                <div class="float-end pb-4">
                  <UButton label="Cancelar" icon="i-heroicons-x-mark-20-solid" @click="isOpenModal = false"
...
const { authenticated, dropitems: storeDropitems } = storeToRefs(authStore);
const { logUserOut } = useAuthStore();

const fullName = computed(() => authenticated.value ? useCookie('fullname').value : '');
const userName = computed(() => authenticated.value ? useCookie('username').value : '');
const avatar = computed(() => authenticated.value ? useCookie('avatar').value : '');
const isRol = computed(() => authenticated.value ? useCookie('rol').value : '');
...
const finalDropItems = computed(() => {
  const baseItems = [
    [{
      label: fullName.value || '', // Ensure label is always a string
      slot: 'account',
      disabled: true
    }],
    [{
    ...
    // Aquí se insertarán los items de configuración si existen
    [{
      label: 'Cerrar sesión',
  ...
  if (authenticated.value && storeDropitems.value && storeDropitems.value.length > 0) {
    // Inserta los items del store (ej. Configuración) en la penúltima posición
    baseItems.splice(2, 0, storeDropitems.value);
  }
...
/* if (authenticated) {
  dropItems.splice(2, 0, dropitems)
} */
