        ...
        <UContainer :ui="{ constrained: 'max-w-screen-2xl' }">
            ...
            <div class="grid gap-4">
                <Toolbar class="mb-1 border-none">
                    <template #start>
                        <UButton @click="isOpenModalGeneral = true" icon="i-material-symbols-add-circle" size="md"
                            square variant="soft">Agregar medio de pago</UButton>
                    </template>
                </Toolbar>
                    ...
                    <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
                        <template #header>
                            <div class="flex items-center justify-between">
                                ...
                                <UButton color="gray" variant="ghost" icon="i-heroicons-x-mark-20-solid" class="-my-1"
                                    @click="isOpenModalGeneral = false" />
                            </div>
                        </template>
                        <div class="flex flex-col gap-6">
                            ...
                            <div>
                                ...
                                <USelect v-model="agregarDatosMP.tipodepago" :options="tipopago"
                                    option-attribute="label" value-attribute="value" fluid></USelect>
                            </div>
                            <div>
                                <label for="estado" class="block font-bold mb-3">Estado</label>
                                <USelect v-model="agregarDatosMP.estado" :options="statuses" option-attribute="label"
                                    value-attribute="value" />
                            </div>
                            <div>
                                <label for="dias" class="block font-bold mb-3">Días de la semana</label>
                                <USelectMenu v-model="agregarDatosMP.dias" :options="dias" option-attribute="label"
                                    multiple by="id">
                                    <template #label>
                                        <span v-if="agregarDatosMP.dias.length" class="truncate">{{
                                            agregarDatosMP.dias.map((data:any) => data.label).toString() }}</span>
                                        <span v-else>Elegir días</span>
                                    </template>
                                </USelectMenu>
                            </div>
                            <div>
                                <label for="financiacion" class="block font-bold mb-3">Mostrar en Costo de
                                    financiación</label>
                                <USelect v-model="agregarDatosMP.financiacion" :options="mostrar"
                        ...
                        <template #footer>
                            <div class="float-end pb-4">
                                <UButton label="Cancelar" icon="i-heroicons-x-mark-20-solid"
                                    @click="isOpenModalGeneral = false" variant="link" color="black" />
                                <UButton label="Guardar" icon="i-heroicons-check" @click="createMedioPago()"
                                    variant="soft" />
                            </div>
                        </template>
                    </UCard>
                </UModal>
                <DataTable :expandedRows="expandedRows" @update:expandedRows="onExpandedRows" sortField="tipo_pago"
                    :sortOrder="1" v-model:editingRows="editingRows" :value="data_mediosdepagos"
                    @row-expand="onRowExpand" editMode="row" dataKey="id" @row-edit-save="onRowEditSave" :pt="{
                        table: { class: 'w-full' },
                    ...
                    <Column field="tipo_pago" header="Tipo de pago" sortable>
                        <template #editor="{ data, field }">
                            <USelect v-model="data[field]" :options="tipopago" option-attribute="label"
                                value-attribute="value" placeholder="Seleccione un tipo de pago" fluid />
                        </template>
                    </Column>
                        ...
                        <template #editor="{ data, field }">
                            <Select v-model="data[field]" :options="statuses" optionLabel="label" optionValue="value"
                                ...
                                <template #option="slotProps">
                                    <Tag :value="slotProps.option.value == true ? 'HABILITADO' : 'INHABILITADO'"
                                        :severity="getStatusLabel(slotProps.option.value)" />
                                </template>
                            </Select>
                        </template>
                        <template #body="slotProps">
                            <Tag :value="slotProps.data.estado == true ? 'HABILITADO' : 'INHABILITADO'"
                                :severity="getStatusLabel(slotProps.data.estado)" />
                        </template>
                    </Column>
                    <Column field="costo_financiacion" header="Mostrar en Costo de financiación?">
                        <template #editor="{ data, field }">
                            <Select v-model="data[field]" :options="mostrar" optionLabel="label" optionValue="value"
                                placeholder="Seleccione un estado" fluid>
                                <template #option="slotProps">
                                    <Tag :value="slotProps.option.value == true ? 'SI' : 'NO'"
                                        :severity="getMostrarLabel(slotProps.option.value)" />
                                </template>
                            </Select>
                        </template>
                        <template #body="slotProps">
                            <Tag :value="slotProps.data.costo_financiacion == true ? 'SI' : 'NO'"
                                :severity="getMostrarLabel(slotProps.data.costo_financiacion)" />
                        </template>
                    </Column>
                    <Column field="dias" header="Días">
                        ...
                        <template #body="slotProps">
                            ...
                            <UBadge color="gray" variant="soft" size="md" class="m-1 rounded-lg"
                                v-if="slotProps.data.dias" v-for="item of slotProps.data.dias" :key="item.id">
                    ...
                    <template #expansion="slotProps">
                        <div class="p-4">
                            <Toolbar class="mb-1 border-none">
                                <template #start>
                                    <UButton @click="isOpenModal = true" icon="i-material-symbols-add-circle" size="sm"
                                        square variant="soft">Agregar</UButton>
                                </template>
                            </Toolbar>
                            <DataTable :value="slotProps.data.interes_base" resizableColumns columnResizeMode="expand"
                                stripedRows sortField="nro_cuota" :sortOrder="1" editMode="cell"
                                v-model:editingRows="editingRowsGroup" :dataKey="slotProps.data.id.toString()"
                                ...
                                <Column header="Una fact. y varios medios pago">
                                    <template #body="{ data, field }">
                                        <Tag :value="geUnaFactVariosMP(data.interes_base)"
                                            :severity="getStatus(data.interes_base)" />
                                    </template>
                                </Column>
                                <Column style="width: 5%;" bodyStyle="text-align:left">
                                    <template #body="{ data, index }">
                                        <UButton class="flex-none" icon="i-heroicons-trash-solid" size="sm"
                                            variant="link" color="red" squre @click="removeIdInteresBase(data.uuid)">
                                        </UButton>
                                    </template>
                                </Column>
                            </DataTable>
                                ...
                                <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
                                    ...
                                    <template #footer>
                                        <div class="float-end pb-4">
                                            ...
                                            <UButton label="Guardar" icon="i-heroicons-check"
                                                @click="createInteresBase(slotProps.data.id)" variant="soft" />
                                        </div>
                                    </template>
                                </UCard>
                            </UModal>
...
definePageMeta({
    title: 'Medios de Pago - Simulador de cobranzas',
