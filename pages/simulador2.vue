  ...
  <div>
    <UContainer :ui="{ constrained: 'max-w-screen-2xl' }">
      ...
      <div class="flex justify-center mb-8">
        ...
        <UTooltip name="buscarnombre" text="Buscar por nombre" :shortcuts="['S']">
          ...
          <UModal v-model="isOpen2">
            <UCommandPalette placeholder="" :autoselect="false" v-model="selected2" @update:model-value="onSelect"
              :groups="groups"
        ...
        <USelectMenu icon="i-heroicons-funnel-solid" trailingIcon="i-heroicons-arrows-up-down-20-solid" color="primary"
          size="xl" variant="none" :options="['Cliente', 'Factura', 'Remito']" v-model="cambiarinput"></USelectMenu>

        <template v-if="cambiarinput == 'Factura'">
          <UButtonGroup size="xl" orientation="horizontal"
            :ui="{ base: 'appearance-none', rounded: 'rounded-lg', form: 'form-input' }">
            <UInput name="search" :ui="{ base: 'w-64' }" size="xl" type="number" icon="octicon:number-24"
              placeholder="Ingrese el nro. de factura" v-model="datainputfactura" />
        ...
        <template v-if="cambiarinput == 'Remito'">
          <UButtonGroup size="xl" orientation="horizontal"
            :ui="{ base: 'appearance-none', rounded: 'rounded-lg', form: 'form-input' }">
            <UInput name="search" :ui="{ base: 'w-64' }" size="xl" padded type="number" icon="octicon:number-24"
              placeholder="Ingrese el nro. de remito" v-model="datainputremito" />
        ...
        <template v-if="cambiarinput == 'Cliente'">
          <UButtonGroup size="xl" orientation="horizontal"
            :ui="{ base: 'appearance-none', rounded: 'rounded-lg', form: 'form-input' }">
            <UInput name="search" :ui="{ base: 'w-64' }" size="xl" padded type="number" icon="octicon:number-24"
              placeholder="Ingrese el nro. de cliente" v-model="datainputcliente" />
        ...
        <template v-if="getdata.data">
          <UTooltip name="clear-all" :shortcuts="['B']">
            ...
            <UButton size="lg" class="ml-2.5" icon="pajamas:clear-all" variant="link" color="gray"
              @click="reloadNuxtApp()">
            </UButton>
          </UTooltip>
        </template>
      ...
      <div class="tab table-wrapper mb-4">
        <template v-if="getdata.data.length > 0">
          <UCard class="w-full" :ui="{
            ...
            <UTable :ui="tableUI" v-model="selected" :rows="getdata.data" :columns="columns"
              @select:all="onHandleSelectAll" @update:modelValue="onUpdateSelection">
              <template #caption>
              ...
              <template #DTO_FINANCIERO-data="{ row }">
                <UBadge size="lg" :label="row.DTO_FINANCIERO + '%'"
                  :color="row.DTO_FINANCIERO >= 0 ? 'emerald' : 'orange'" variant="soft" />
              </template>
            ...
            <template #footer>
              <div class="flex flex-wrap justify-between items-center">
                ...
                <div>
                  <span>Final «CONTADO EFECTIVO», antes considerar medios pago: $ {{
                    formatterNumber.format(formData.monto) }}</span>
                </div>
              </div>
            </template>
          </UCard>
        ...
        <template v-if="getdata.dataNC.length > 0">
          <UCard class="w-full" :ui="{
            ...
            <UTable :ui="tableUI" v-model="selectedNC" :rows="getdata.dataNC" :columns="columns2"
              @select:all="onHandleSelectAll2" @update:modelValue="onUpdateSelection2">
              <template #caption>
          ...
          <Toolbar class="my-4 border-none dark:bg-transparent">
            <template #end>
              <div class="flex items-center">
                <UBadge size="lg" variant="solid">Total FACTURA - NOTA DE CRÉDITO: $ {{
                  formatterNumber.format(Number(formData.monto) - Number(formData.montoNC)) }} </UBadge>
              </div>
            </template>
          </Toolbar>
        </template>
      ...
      <div class="flex justify-center">
        <template v-if="pagocerrado">
          <div class="flex flex-col w-full ">
            <UDivider label="¡¡PAGO CERRADO!!" type="dashed"
              :ui="{ label: 'text-sea-green-600 dark:text-sea-green-400 text-lg mb-2' }" />
                  ...
                  <template v-if="formData.totalimporte >= formData.montofactura">
                    ...
                    <div class="rounded-md p-2 text-xl text-center bg-red-500 dark:bg-red-600">
                      {{ formatterNumber.format((formData.totalimporte -
                        Number(formData.montofactura)) / Number(formData.montofactura) * 100) }} %
                    </div>
                    <div><span>FINAL FACTURAR + DÉBITO</span></div>
                    <div
                      class="rounded-md p-2 text-xl text-center bg-primary-500 dark:bg-primary-400 text-white font-medium dark:text-gray-900">
                      $ {{ formatterNumber.format(Number(formData.montofactura) + (formData.totalimporte -
                        Number(formData.montofactura))) }}
                    </div>
                    <template v-if="formData.montoNC">
                      ...
                      <div
                        class="rounded-md p-2 text-xl text-center bg-primary-500 dark:bg-primary-400 text-white font-medium dark:text-gray-900">
                        $ {{ formatterNumber.format(Number(formData.montofactura) + (formData.totalimporte -
                          Number(formData.montofactura)) - Number(formData.montoNC)) }}
                      </div>
                    </template>
                  </template>
                  <template v-else>
      ...
      <template v-if="formData.monto">
        <UButton @click="addMore" icon="i-material-symbols-add-circle" size="md" square variant="soft" class="my-4">
          Agregar</UButton>
        <div class="grid grid-rows-auto grid-flow-col gap-4">
          <div class="row-span-3">
            <div class="grid grid-flow-col auto-rows-auto auto-cols-max gap-4 mb-2 p-2 items-end"
              v-for="(item, i) in formData.mediospago" :key="i">
              ...
              <template v-if="data_mediosdepagos">
                <UFormGroup label="Medio de pago">
                  <USelect color="white" variant="outline" placeholder="" :options="dataMediosPagos"
                    option-attribute="name" v-model="item.nombre" />
                </UFormGroup>
              </template>
              <template
                v-if="data_mediosdepagos?.filter(data => data.nombre == item.nombre).map(x => x.tipo_pago)[0] == 'TC'">
                <UFormGroup label="Nro. cuota">
                  <USelect color="white" variant="outline" placeholder=""
                    :options="data_mediosdepagos?.filter(data => data.nombre == item.nombre).map((data: any) => data.interes_base)[0].map((x: any) => x.nro_cuota)"
                    v-model="item.cuota" />
                </UFormGroup>
              </template>
              ...
              <UFormGroup label="Importe a pagar">
                <UInput v-model="item.importe" variant="outline" placeholder="" icon="clarity:dollar-line"
                  v-maska="optionsMask">
                </UInput>
              </UFormGroup>
              <UButton class="flex-none" title="Borrar item" @click="remove(i)" icon="i-heroicons-trash" size="sm"
                square variant="soft"></UButton>
              <UButton class="flex-none" title="Duplicar item" @click="copyItem(i)" icon="i-heroicons-clipboard"
                size="sm" square variant="soft"></UButton>
            </div>
          </div>
          <div class="col-span-2">
            <div class="grid grid-flow-col auto-rows-auto auto-cols-max gap-4 mb-2 p-2 items-end"
              v-for="(item, y) in formData.mediospago" :key="y">
              ...
              <div v-if="!item.importe">
                <template v-if="formData.monto && item.dto">
                  <template v-if="Number(item.calculable) > 0">
                    <UFormGroup label="Importe deseable"
                      @click="toast.add({ title: 'Copiado!' }); copy(item.calculable)">
              ...
              <div>
                <template v-if="item.importe">
                  <template
                    v-if="data_mediosdepagos?.filter(data => data.nombre == item.nombre).map(x => x.tipo_pago)[0] == 'TC'">
                    <UBadge size="lg" color="amber">{{ !item.importe ? item.cuota + ' CUOTAS DE $ ' +
                      formatterNumber.format(Number(formData.monto.replaceAll(",", "")) / Number(item.cuota)) :
                      item.cuota + ' CUOTAS DE $ ' + formatterNumber.format((Number(item.importe.replaceAll(",", "")) /
                        Number(item.cuota))) }}</UBadge>
                  </template>
                  <template
                    v-else-if="data_mediosdepagos?.filter(data => data.nombre == item.nombre).map(x => x.tipo_pago)[0] !== 'TC'">
                    <UBadge size="lg" color="amber">{{ item.nombre == "MAESTRO" || item.nombre == "VISA ELECTRON" ||
                      item.nombre == "CABAL DEBITO" ? 'DÉBITO $ ' + item.importe : item.nombre + ' $ ' + item.importe }}
                    </UBadge>
                  </template>
                </template>
              </div>
              <div>
                <template v-if="pagocerrado">
                  <template
                    v-if="data_mediosdepagos?.filter(data => data.nombre == item.nombre).map(x => x.tipo_pago)[0] == 'TC'">
                    <UButton class="flex-none" title="Copiar"
                      @click="toast.add({ title: 'Copiado!' }); copy(!item.importe ? item.nombre + '-' + item.cuota + ' CUOTAS DE $ ' + formatterNumber.format(Number(formData.monto.replaceAll(',', '')) / Number(item.cuota)) : item.nombre + ' - Total $ ' + item.importe + ' - Cada uno ' + item.cuota + ' CUOTAS DE $ ' + formatterNumber.format((Number(item.importe.replaceAll(',', '')) / Number(item.cuota))))"
                      icon="i-heroicons-clipboard-document" size="sm" square variant="soft"></UButton>
                  </template>
                  <template
                    v-else-if="data_mediosdepagos?.filter(data => data.nombre == item.nombre).map(x => x.tipo_pago)[0] !== 'TC'">
                    <UButton class="flex-none" title="Copiar"
                      @click="toast.add({ title: 'Copiado!' }); copy(item.nombre == 'MAESTRO' || item.nombre == 'VISA ELECTRON' || item.nombre == 'CABAL DEBITO' ? 'DÉBITO $ ' + item.importe : item.nombre + ' $ ' + item.importe)"
                      icon="i-heroicons-clipboard-document" size="sm" square variant="soft"></UButton>
                  </template>
                </template>
              </div>
            ...
            <template #header>
              <div class="flex items-center justify-between">
                <h3 class="text-base flex justify-center font-semibold leading-6 text-gray-900 dark:text-white">
                  <UIcon :name="'i-heroicons-exclamation-circle'" size="md"
                    class="h-6 w-6 mr-2 text-red-400 dark:text-red-500" />
                  Advertencia
                </h3>
                <UButton color="gray" variant="ghost" icon="i-heroicons-x-mark-20-solid" class="-my-1"
                  @click="isOpen = false" />
...
const consultaSaldosRemito = async () => {
  try {
    ...
    const response: any = await $fetch(config.public.apiBase + '/consultasaldosctacteremito', {
      method: 'GET',
      headers: {
        Accept: "application/json",
        'Authorization': 'Bearer ' + config.public.apiToken
