  ...
  <div>
    <UContainer :ui="{ constrained: 'max-w-screen-2xl' }">
      ...
      <div class="flex justify-center">
        <UInput :ui="{ rounded: 'rounded-lg' }" size="xl" icon="clarity:dollar-line" class="mb-8 max-w-2xl "
          placeholder="Ingresar monto" v-model="monto" v-maska="optionsMask">
          <template #trailing>
      ...
      <div class="tab">
        <template v-if="promoDia">
          <UDivider label="PROMOCIONES DEL DÍA" type="dashed"
              ...
              <div class="text-sea-green-900 dark:text-white">
                <UTable v-model:sort="sort" :columns="selectedColumns" :rows="item.data" class="w-full">
                  <template #porc_dto_deb-data="{ row }">
                    <UBadge :label="row.porc_dto_deb + '%'" variant="soft"
                      :color="row.porc_dto_deb > 0 ? 'green' : 'red'" />
                  </template>
                  <template #dto_deb-data="{ row }">
                    <UBadge :label="'$ ' + formatter_decimal.format(row.dto_deb)" variant="soft"
                      :color="row.dto_deb > 0 ? 'green' : 'red'" />
                  </template>
                </UTable>
              </div>
            </template>
...
definePageMeta({
  title: 'Tarjetas vista general - Simulador de cobranzas',
})
const monto = ref()
