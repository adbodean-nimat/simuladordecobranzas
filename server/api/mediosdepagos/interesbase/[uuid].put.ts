...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (!result[0]) {
      ...
      const NroCuotabyUUID = result[0].interes_base
        .filter((item: any) => uuid == item.uuid)
        .map((data: { nro_cuota: any }) => data.nro_cuota)[0];
      const InteresBasebyUUID = result[0].interes_base
        .filter((item: any) => uuid == item.uuid)
        .map((data: { interes_base: any }) => data.interes_base)[0];
      const indexData = result[0].interes_base.map(
        (data: any[], index: any) => ({ index, data })
      );
      const uuidById = indexData
        .filter((item: any) => uuid == item.data.uuid)
        .map((data: any, index: any) => data.index)[0];
      const uuidByIndex = Prisma.sql([uuidById]);
      const json = {
        uuid: uuid,
        nro_cuota:
          body.nro_cuota === "" ||
          body.nro_cuota === undefined ||
          Number.isNaN(body.nro_cuota)
            ? NroCuotabyUUID
            : body.nro_cuota,
        interes_base:
          body.interes_base === "" ||
          body.interes_base === undefined ||
          Number.isNaN(body.interes_base)
            ? InteresBasebyUUID
            : body.interes_base,
      };
