...
export default defineEventHandler(async (event) => {
  try {
    ...
    const json = {
      uuid: uuidv1(),
      nro_cuota: body.nro_cuota,
      interes_base: body.interes_base,
    };
    const result: any =
      await prisma.$queryRaw`SELECT * FROM public.simulador_cobranzas_medios_de_pagos WHERE id = ${body.id}`;
    var datosInteresBaseFiltroxID: any = [];
    const getData: any = result.forEach((item: { interes_base: any[] }) =>
      item.interes_base?.map((data) => datosInteresBaseFiltroxID.push(data))
    );
    ...
    if (
      datosInteresBaseFiltroxID.findIndex(
        (e: { nro_cuota: string }) => e.nro_cuota === body.nro_cuota
      ) == -1
    ) {
